...
龙波是一个零订阅费用的 BTC 周期监控网页。它展示 BTC 价格、NUPL、已实现价格、MVRV、MVRV Z-Score、杠杆代理和狼波周期指数，并通过 Gmail 发送每日报告与高风险状态预警。

页面会自动找出 `2011`、`2015`、`2018`、`2022` 四轮周期内的 BTC 日线最低点，并将当日全部可用指标与当前数据并排展示。杠杆代理没有口径一致的免费跨周期历史数据，因此对应位置明确标记为无历史数据。
